            ...
            <div className="p-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/95 backdrop-blur z-20 sticky top-0">
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`p-2 rounded-lg transition-all ${viewMode === 'day' ? 'bg-zinc-800 text-white' : 'text-zinc-500'}`}
                        title="Day View"
                    >
                        <List size={20} />
                    </button>
                    <button
                        ...
                        className={`p-2 rounded-lg transition-all ${viewMode === 'month' ? 'bg-zinc-800 text-white' : 'text-zinc-500'}`}
                        title="Month View"
                    >
                        <LayoutGrid size={20} />
                    </button>
                ...
                {viewMode === 'day' && (
                    <div className="flex items-center gap-2 bg-zinc-800 rounded-lg p-1">
                        ...
                        <span className="text-xs font-bold w-24 text-center text-white">
                            {isSameDate(selectedDate, new Date()) ? 'TODAY' : selectedDate.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}
                        </span>
                        <button
                            onClick={() => changeDay(1)}
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="flex items-center gap-2 px-3 md:px-5 py-2 md:py-2.5 bg-lime-400 text-black rounded-xl hover:bg-lime-300 transition-all shadow-[0_0_15px_rgba(163,230,53,0.3)] text-sm font-bold hover:scale-105 active:scale-95"
                    >
...
export default function App() {
    ...
    return (
        <ErrorBoundary>
            <div className="min-h-screen bg-black text-zinc-100 font-sans flex flex-col selection:bg-lime-400 selection:text-black overflow-x-hidden">
                        ...
                        <div className="hidden md:flex items-center gap-1 bg-black p-1 rounded-xl border border-zinc-800">
                            {['stats', 'grid', 'squad'].map(v => (
                                <button
                                    key={v}
                                    onClick={() => setView(v)}
                                    className={`px-4 py-2 rounded-lg text-sm font-bold uppercase transition-all ${view === v ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-white'}`}
                                >
                                    {v}
                                </button>
                            ))}
                ...
                {/* Main Content Area */}
                <main className="flex-1 max-w-7xl mx-auto w-full p-2 md:p-6 print:p-0 print:max-w-none flex flex-col pb-24 md:pb-6">
