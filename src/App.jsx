...
import {
    Plus, Trash2, BarChart2, Grid, ChevronLeft, ChevronRight, Check,
    Smartphone, Activity, Target, Trophy, Layout, Save, Menu, X, LogOut,
    LogIn, Zap, Flame, TrendingUp, Lock, Unlock, Bell, Calendar, Maximize2,
    Minimize2, Cpu, Globe, ShieldCheck, Users, Crown, ArrowRight, UserCircle,
    Copy, AlertTriangle, Download, ShieldAlert, Medal, Sparkles, Share,
    Camera, Star, Award, Moon, Briefcase, GraduationCap, Sword, Book,
    ArrowDown, Clock, Sun, Sunset, Sunrise, Repeat, Skull, ZapOff, BookOpen,
    HelpCircle, Navigation, Gift, Loader2, List, LayoutGrid
} from 'lucide-react';
import {
...
// iOS Safari Safe Storage Wrapper (Prevents crashes in Private Mode)
const safeStorage = {
    get: (key) => {
...
class ErrorBoundary extends React.Component {
    ...
    render() {
        if (this.state.hasError) {
            return (
                <div className="min-h-screen w-full bg-black text-red-500 p-8 flex flex-col items-center justify-center text-center font-mono">
...
const ARCHETYPES = {
    MONK: {
        id: 'monk', title: 'The Monk', problem: "Distraction & Anxiety", solution: "Clarity & Peace",
        icon: Moon, color: 'text-indigo-400', manifesto: "I seek clarity in a chaotic world. Stillness is my strength.",
        defaults: [
    ...
    WARRIOR: {
        id: 'warrior', title: 'The Warrior', problem: "Weakness & Lethargy", solution: "Strength & Power",
        icon: Sword, color: 'text-red-500', manifesto: "I do not negotiate with myself. Pain is weakness leaving the body.",
        defaults: [
    ...
    FOUNDER: {
        id: 'founder', title: 'The Founder', problem: "Stagnation & Poverty", solution: "Growth & Wealth",
        icon: Briefcase, color: 'text-emerald-400', manifesto: "I build the future. Every second is an investment.",
        defaults: [
    ...
    SCHOLAR: {
        id: 'scholar', title: 'The Scholar', problem: "Ignorance & Confusion", solution: "Knowledge & Wisdom",
        icon: GraduationCap, color: 'text-yellow-400', manifesto: "I am a perpetual learner. Knowledge is my compounding asset.",
        defaults: [
...
const ACHIEVEMENTS = [
    ...
    { id: 'mastery', name: 'Protocol Master', desc: 'Complete 100 total reps', icon: Trophy, condition: (habits) => habits.reduce((acc, h) => acc + Object.keys(h.history || {}).length, 0) >= 100 },
    { id: 'dedication', name: 'Full House', desc: '5 Active Habits', icon: Grid, condition: (habits) => habits.length >= 5 }
];

const LoadingScreen = ({ message }) => (
    <div className="min-h-screen w-full bg-black flex flex-col items-center justify-center p-4 text-white font-sans">
        <div className="animate-spin text-lime-400 mb-4">
            <Loader2 size={48} />
        </div>
        <p className="text-zinc-500 font-mono text-sm tracking-widest animate-pulse">{message}</p>
    </div>
);
...
const ReflectionModal = ({ isOpen, onClose, onRecover, tokens }) => {
    ...
    const handleRecover = () => {
        if (tokens <= 0) {
            setErrorMsg("INSUFFICIENT TOKENS.");
                    ...
                    <button
                        ...
                        className={`w-full py-3 border font-bold rounded-lg transition-all flex items-center justify-center gap-2
                            ${tokens > 0
                                ...
                                : 'bg-zinc-800 border-zinc-700 text-zinc-600 opacity-50'}
                        `}
                    >
                        <Repeat size={16} /> {tokens > 0 ? "USE TOKEN & REPAIR STREAK" : "INSUFFICIENT TOKENS"}
                    </button>
                    {errorMsg && <p className="text-red-500 text-[10px] text-center mt-2 font-bold animate-pulse">{errorMsg}</p>}
                </div>
...
const UserGuide = ({ isOpen, onClose }) => {
    ...
    if (!isOpen) return null;
    const pages = [
        { title: "WELCOME TO THE GRID", icon: Grid, color: "text-lime-400", content: "You are now operating on Tracker.OS. This is not a todo list. It is a visual database of your consistency. Every box you fill adds to your momentum." },
        { title: "TRACKING PROTOCOLS", icon: Check, color: "text-cyan-400", content: "Click a box on 'The Grid' to mark a habit as complete. Dark cells are future days (locked). Red cells are missed days. Maintain the streak to earn XP." },
        { title: "XP & RANKING", icon: Trophy, color: "text-yellow-400", content: "Every completion grants XP. As you gain XP, your Rank increases from 'Drifter' to 'Supreme'. Unlock badges and prove your discipline." },
        { title: "SQUAD SYNC", icon: Users, color: "text-purple-400", content: "Don't fight alone. Go to the Squad tab to create a family or team leaderboard. Share your code to compete live with friends." }
    ];
        ...
        <div className="fixed inset-0 z-[90] bg-black/95 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-zinc-900 w-full max-w-lg rounded-3xl border border-zinc-800 overflow-hidden shadow-2xl relative">
                ...
                <div className="h-2 bg-zinc-800 w-full">
                    <div className="h-full bg-lime-400 transition-all duration-300" style={{ width: `${((page + 1) / pages.length) * 100}%` }}></div>
                </div>
                <div className="p-8 text-center min-h-[400px] flex flex-col items-center justify-center">
                    ...
                    <div className="flex items-center gap-4 w-full">
                        {page > 0 && (
                            <button onClick={() => setPage(p => p - 1)} className="flex-1 py-3 rounded-xl bg-zinc-800 text-zinc-400 font-bold hover:bg-zinc-700 transition-colors">
                                BACK
                            </button>
                        )}
                        <button onClick={() => { page < pages.length - 1 ? setPage(p => p + 1) : onClose() }} className="flex-1 py-3 rounded-xl bg-lime-400 text-black font-black hover:bg-lime-300 transition-colors">
...
const OnboardingModal = ({ onComplete }) => {
    ...
    const handleSelect = (id) => { setSelectedArchetype(id); setStep(2); };
    const handleCommit = () => {
    ...
    return (
        <div className="fixed inset-0 z-[70] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="max-w-4xl w-full bg-zinc-900 rounded-3xl overflow-hidden border border-zinc-800 shadow-2xl flex flex-col md:flex-row h-[85vh] max-h-[800px]">
                <div className="w-full md:w-1/3 bg-zinc-950 p-8 flex flex-col justify-between border-r border-zinc-800">
                    <div>
                        ...
                        <p className="text-zinc-400 text-sm leading-relaxed">
                            {step === 1 ? "What problem are you trying to solve? Select the archetype that aligns with your current goal."
                    ...
                    {step === 1 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {Object.values(ARCHETYPES).map((arch) => (
                                <button key={arch.id} onClick={() => handleSelect(arch.id)} className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-900 hover:border-lime-500/50 transition-all text-left group relative overflow-hidden flex flex-col h-full min-h-[280px]">
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center">
                            ...
                            <p className="text-zinc-400 max-w-md mb-8">We are about to install 3 core habits into your operating system.</p>
                            <div className="flex gap-4">
                                ...
                                <button onClick={handleCommit} className="px-8 py-3 bg-lime-400 text-black font-black rounded-xl hover:bg-lime-300 hover:scale-105 transition-all shadow-[0_0_20px_rgba(163,230,53,0.3)]">
                                    BEGIN PROTOCOL
                                </button>
                            </div>
                        </div>
...
const NewHabitModal = ({ isOpen, onClose, onConfirm }) => {
    ...
    const handleModeSelect = (selectedMode) => {
        if (selectedMode === 'motivation') setShowLecture(true);
        else setMode('discipline');
    };

    const handleSubmit = () => {
        if (!name) return;
        onConfirm(name, parseInt(goal) || 0, timeOfDay);
        setName(''); setGoal(''); setMode(null); setShowLecture(false); setTimeOfDay('anytime');
        onClose();
    };
            ...
            <div className="bg-zinc-900 border border-zinc-700 w-full max-w-md rounded-2xl p-6 shadow-2xl relative overflow-hidden">
                ...
                {!mode && !showLecture && (
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full bg-black border border-zinc-700 rounded-xl p-3 text-white focus:border-lime-400 outline-none" placeholder="e.g. 5AM Run" />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] uppercase font-bold text-zinc-400 tracking-widest">Time Constraint</label>
                            <div className="grid grid-cols-4 gap-2">
                                {[ { id: 'morning', icon: Sunrise, label: 'AM' }, { id: 'afternoon', icon: Sun, label: 'Mid' }, { id: 'evening', icon: Sunset, label: 'PM' }, { id: 'anytime', icon: Clock, label: 'Any' }].map(t => (
                                    <button key={t.id} onClick={() => setTimeOfDay(t.id)} className={`flex flex-col items-center justify-center p-2 rounded-xl border transition-all ${timeOfDay === t.id ? 'bg-zinc-800 border-lime-400 text-lime-400' : 'bg-black border-zinc-700 text-zinc-500'}`}>
                                        <t.icon size={16} />
                        ...
                        <div className="space-y-2">
                            ...
                            <input type="number" value={goal} onChange={e => setGoal(e.target.value)} className="w-full bg-black border border-zinc-700 rounded-xl p-3 text-white focus:border-lime-400 outline-none" placeholder="0 = Every Day" />
                        </div>
                        <div className="pt-4">
                            <p className="text-center text-zinc-300 text-sm font-bold mb-4">What fuels this habit?</p>
                            <div className="grid grid-cols-2 gap-3">
                                <button onClick={() => handleModeSelect('motivation')} className="p-4 rounded-xl bg-gradient-to-br from-pink-500/20 to-purple-500/20 border border-pink-500/30 hover:border-pink-500 text-pink-200 font-bold text-sm transition-all">
                                    Motivation <span className="block text-[9px] font-normal opacity-70 mt-1">"I feel like doing it"</span>
                                </button>
                                <button onClick={() => handleModeSelect('discipline')} className="p-4 rounded-xl bg-gradient-to-br from-lime-500/20 to-emerald-500/20 border border-lime-500/30 hover:border-lime-500 text-lime-200 font-bold text-sm transition-all">
                                    Discipline <span className="block text-[9px] font-normal opacity-70 mt-1">"I must do it"</span>
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                {showLecture && (
                    <div className="text-center py-8 animate-in zoom-in-95">
                        ...
                        <p className="text-zinc-300 text-sm leading-relaxed mb-6">Motivation is a feeling. Feelings are temporary.<br />Discipline is a system. Systems last forever.</p>
                        <button onClick={() => setShowLecture(false)} className="px-6 py-2 bg-zinc-800 text-white text-xs font-bold rounded-lg hover:bg-zinc-700 transition-colors border border-zinc-600">
                            GO BACK
                        </button>
                    </div>
                )}
                {mode === 'discipline' && !showLecture && (
                    <div className="text-center py-6 animate-in slide-in-from-right">
                        ...
                        <p className="text-zinc-400 text-sm mb-6">You have chosen the hard path. Good.</p>
                        <button onClick={handleSubmit} disabled={!name} className="w-full bg-lime-400 text-black font-black py-4 rounded-xl hover:bg-lime-300 transition-colors">
                            CONFIRM & COMMIT
                        </button>
                    </div>
                )}
...
const LoginView = ({ onCloudSync, onLocalMode, error, isConfigured }) => {
    ...
    return (
        <div className="min-h-screen w-full bg-black flex flex-col items-center justify-center p-4 text-white relative overflow-hidden font-sans">
                ...
                <div className="w-full space-y-3 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="relative">
                            ...
                            <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="Name" className="w-full bg-black/50 border border-zinc-700 rounded-xl py-3 pl-10 pr-3 text-sm text-white placeholder-zinc-600 focus:border-lime-400 focus:ring-0 outline-none transition-all" />
                        </div>
                        <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Surname" className="w-full bg-black/50 border border-zinc-700 rounded-xl py-3 px-4 text-sm text-white placeholder-zinc-600 focus:border-lime-400 focus:ring-0 outline-none transition-all" />
                    </div>
                    {validationMsg && <p className="text-red-400 text-[10px] uppercase font-bold tracking-wide text-center animate-pulse">{validationMsg}</p>}
                </div>
                
                <div className="w-full space-y-4">
                    <button onClick={() => validateAndSubmit(onCloudSync)} disabled={!isConfigured || isProcessing} className={`w-full font-black py-4 px-6 rounded-2xl transition-all flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(255,255,255,0.1)] group ${isConfigured && !isProcessing ? 'bg-white text-black hover:bg-zinc-200 hover:scale-[1.02] active:scale-[0.98]' : 'bg-zinc-800 text-zinc-500 cursor-not-allowed'}`}>
                        {isProcessing ? <><Loader2 size={20} className="animate-spin" /> CONNECTING...</> : <><Globe size={20} className="group-hover:rotate-12 transition-transform" /> {isConfigured ? 'INITIALIZE CLOUD SYNC' : 'CLOUD UNAVAILABLE'}</>}
                    </button>
                    <div className="relative py-2">
                        <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-white/10" /></div>
                        <div className="relative flex justify-center text-[10px] uppercase font-bold tracking-widest"><span className="bg-transparent px-3 text-zinc-500">Or Access Local Storage</span></div>
                    </div>
                    <button onClick={() => validateAndSubmit(onLocalMode)} disabled={isProcessing} className="w-full bg-black/50 hover:bg-black/70 text-zinc-300 font-bold py-4 px-6 rounded-2xl transition-all flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-[0.98] border border-white/10 group disabled:opacity-50">
                        <Cpu size={20} className="text-lime-400 group-hover:animate-spin-slow" /> DEVICE STORAGE MODE
                    </button>
                </div>
                
                {error && <div className="mt-6 p-4 bg-red-500/10 border border-red-500/30 rounded-2xl text-red-400 text-xs text-center font-bold uppercase tracking-wide flex items-center gap-2"><ShieldCheck size={16} /> {error}</div>}
                {!isConfigured && <p className="mt-4 text-[10px] text-zinc-600 text-center max-w-xs leading-relaxed">Database link not detected. Cloud features disabled.</p>}
            </div>
        </div>
    );
};

const SquadView = ({ user, userProfile, onJoinSquad, appMode }) => {
    const [joinCode, setJoinCode] = useState('');
    const [members, setMembers] = useState([]);
    const [activeTab, setActiveTab] = useState('join');

    useEffect(() => {
        if (!userProfile?.squadId) return;
        if (appMode === 'local') {
            setMembers([{
                uid: 'local-user',
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const mems = snapshot.docs.map(doc => ({ uid: doc.id, ...doc.data() }));
            mems.sort((a, b) => (b.todayScore || 0) - (a.todayScore || 0));
            setMembers(mems);
        });
        return () => unsubscribe();
    }, [appMode, userProfile?.squadId]);

    const handleJoin = (e) => {
        e.preventDefault();
        if (joinCode.trim()) onJoinSquad(joinCode.trim().toUpperCase());
    };

    const handleCreate = () => {
        onJoinSquad('SQ-' + Math.random().toString(36).substring(2, 8).toUpperCase());
    };

    const copySquadCode = () => {
        if (userProfile?.squadId && navigator.clipboard) {
            try { navigator.clipboard.writeText(userProfile.squadId); } catch (e) {}
        }
    };

    if (!userProfile?.squadId) {
        return (
            <div className="flex flex-col items-center justify-center h-full min-h-[500px] animate-in fade-in slide-in-from-bottom-4 p-4 pb-20">
                <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl max-w-md w-full text-center relative overflow-hidden shadow-2xl">
                    ...
                    {appMode === 'local' && (
                        <div className="mb-6 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg text-yellow-500 text-xs font-bold uppercase tracking-wide">
                            ⚠ Local Mode Active: You will only see your own stats. Sign in to cloud sync with others.
                        </div>
                    )}
                    <div className="grid grid-cols-2 gap-2 bg-black p-1 rounded-xl mb-6">
                        <button onClick={() => setActiveTab('join')} className={`py-2 text-xs font-bold uppercase rounded-lg transition-all ${activeTab === 'join' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>Join Squad</button>
                        <button onClick={() => setActiveTab('create')} className={`py-2 text-xs font-bold uppercase rounded-lg transition-all ${activeTab === 'create' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>Create Squad</button>
                    </div>
                    {activeTab === 'join' ? (
                        <form onSubmit={handleJoin} className="space-y-4 animate-in fade-in zoom-in-95">
                            <div>
                                ...
                                <input type="text" value={joinCode} onChange={(e) => setJoinCode(e.target.value)} placeholder="e.g. SQ-XY92Z" className="w-full bg-black border border-zinc-700 rounded-xl p-4 text-center font-black text-xl text-white uppercase placeholder-zinc-800 focus:border-lime-400 focus:ring-0 outline-none transition-all" />
                            </div>
                            <button type="submit" className="w-full bg-white text-black font-black py-4 rounded-xl hover:bg-lime-400 transition-colors flex items-center justify-center gap-2">
                                {appMode === 'local' ? 'ACTIVATE SOLO SQUAD' : 'JOIN SQUAD'} <ArrowRight size={18} strokeWidth={3} />
                            </button>
                        </form>
                    ) : (
                        <div className="space-y-4 animate-in fade-in zoom-in-95">
                            <div className="bg-black/50 p-4 rounded-xl border border-zinc-800/50">
                                ...
                                <div className="flex items-center justify-center gap-2 text-lime-400/50"><ShieldCheck size={20} /><span className="text-xs font-mono">ENCRYPTED CHANNEL</span></div>
                            </div>
                            <button onClick={handleCreate} className="w-full bg-lime-400 text-black font-black py-4 rounded-xl hover:bg-lime-300 transition-colors flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(163,230,53,0.3)]">
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 pb-20">
            <div className="bg-zinc-900 border-b border-zinc-800 p-6 rounded-t-3xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <div className="flex items-center gap-3">
                        <h2 className="text-2xl font-black text-white flex items-center gap-3"><Users className="text-lime-400" /> {userProfile.squadId}</h2>
                        {appMode !== 'local' && (
                            <button onClick={copySquadCode} className="p-1.5 bg-zinc-800 rounded-md text-zinc-400 hover:text-white transition-colors" title="Copy Code"><Copy size={14} /></button>
                        )}
                    </div>
                    <p className="text-zinc-500 text-xs font-bold uppercase tracking-wider mt-1">{appMode === 'local' ? 'Local Performance Record' : 'Live Family Leaderboard'}</p>
                </div>
            ...
            <div className="bg-gradient-to-r from-zinc-900 to-zinc-950 border-x border-zinc-800 p-6 flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="flex gap-4 items-center">
                    <div className="w-12 h-12 bg-zinc-800 rounded-xl flex items-center justify-center border border-zinc-700"><Target className="text-yellow-500" /></div>
                    <div>
            ...
            <div className="bg-zinc-900 border-x border-b border-zinc-800 rounded-b-3xl p-4 space-y-2 min-h-[400px]">
                {members.map((member, index) => (
                    <div key={member.uid} className={`p-4 rounded-2xl flex items-center gap-4 transition-all ${member.uid === user?.uid || (appMode==='local' && member.uid==='local-user') ? 'bg-lime-900/10 border border-lime-500/30' : 'bg-black border border-zinc-800'}`}>
                        ...
                        <div className="w-12 h-12 rounded-full bg-zinc-800 flex items-center justify-center font-bold text-lg border-2 border-zinc-700 overflow-hidden relative shadow-lg">
                            ...
                            {member.lastActive && (Date.now() - member.lastActive.toMillis() < 300000) && <div className="absolute bottom-0 right-0 w-3 h-3 bg-lime-500 rounded-full border-2 border-black"></div>}
                        </div>
                        <div className="flex-1">
                            <h3 className="font-bold text-white flex items-center gap-2 text-sm md:text-base">
                                {member.displayName || 'Unknown Warrior'}
                                {(member.uid === user?.uid || (appMode==='local' && member.uid==='local-user')) && <span className="text-[9px] bg-lime-400 text-black px-1.5 py-0.5 rounded font-black tracking-wider">YOU</span>}
                            </h3>
                            <div className="flex items-center gap-3 mt-1">
                                <div className="h-1.5 flex-1 bg-zinc-800 rounded-full overflow-hidden max-w-[120px]">
                                    <div className="h-full bg-gradient-to-r from-lime-400 to-cyan-400" style={{ width: `${Math.min(100, (member.todayScore || 0) * 10)}%` }}></div>
                                </div>
...
const TrackerView = ({ habits, currentDate, onToggle, onOpenNewHabit, onDelete, onUpdateField, onRepairStreak }) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') setViewMode(window.innerWidth < 768 ? 'day' : 'month');
    }, []);
    ...
    const formattedDateKey = getFullDateKey(selectedDate);

    const getDayLabel = (day) => new Date(currentDate.getFullYear(), currentDate.getMonth(), day).toLocaleDateString('en-US', { weekday: 'narrow' });
    const isToday = (day) => { const today = new Date(); return today.getDate() === day && today.getMonth() === currentDate.getMonth() && today.getFullYear() === currentDate.getFullYear(); };
    const isSameDate = (d1, d2) => d1.getDate() === d2.getDate() && d1.getMonth() === d2.getMonth() && d1.getFullYear() === d2.getFullYear();
    const isFutureDate = (day) => new Date(currentDate.getFullYear(), currentDate.getMonth(), day) > new Date(new Date().setHours(0,0,0,0));
    ...
    const changeDay = (delta) => { const newDate = new Date(selectedDate); newDate.setDate(selectedDate.getDate() + delta); setSelectedDate(newDate); };
    const getDayProgress = () => {
        if (habits.length === 0) return 0;
        const completed = habits.filter(h => h.history?.[formattedDateKey]).length;
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-900 rounded-3xl shadow-2xl border border-zinc-800/50 overflow-hidden relative">
            ...
            <div className="p-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/95 backdrop-blur z-20 sticky top-0">
                <div className="flex items-center gap-2">
                    <button onClick={() => setViewMode('day')} className={`p-2 rounded-lg transition-all ${viewMode === 'day' ? 'bg-zinc-800 text-white' : 'text-zinc-500'}`} title="Day View"><List size={20} /></button>
                    <button onClick={() => setViewMode('month')} className={`p-2 rounded-lg transition-all ${viewMode === 'month' ? 'bg-zinc-800 text-white' : 'text-zinc-500'}`} title="Month View"><LayoutGrid size={20} /></button>
                </div>
                {viewMode === 'day' && (
                    <div className="flex items-center gap-2 bg-zinc-800 rounded-lg p-1">
                        <button onClick={() => changeDay(-1)} className="p-1 hover:text-white text-zinc-400"><ChevronLeft size={16} /></button>
                        <span className="text-xs font-bold w-24 text-center text-white">{isSameDate(selectedDate, new Date()) ? 'TODAY' : selectedDate.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</span>
                        <button onClick={() => changeDay(1)} disabled={new Date(selectedDate).setHours(0,0,0,0) >= new Date().setHours(0,0,0,0)} className="p-1 hover:text-white text-zinc-400 disabled:opacity-30"><ChevronRight size={16} /></button>
                    </div>
                )}
                <button onClick={onOpenNewHabit} className="flex items-center gap-2 px-3 md:px-5 py-2 md:py-2.5 bg-lime-400 text-black rounded-xl hover:bg-lime-300 transition-all shadow-[0_0_15px_rgba(163,230,53,0.3)] text-sm font-bold hover:scale-105 active:scale-95">
                    <Plus size={18} strokeWidth={3} /> <span className="hidden md:inline">NEW HABIT</span>
                </button>
            </div>
                ...
                {viewMode === 'day' && (
                    <div className="max-w-md mx-auto space-y-4 pb-20">
                        <div className="bg-gradient-to-br from-zinc-800 to-zinc-900 p-6 rounded-2xl border border-zinc-700 flex items-center justify-between relative overflow-hidden">
                            ...
                            <div>
                                <p className="text-zinc-400 text-xs font-bold uppercase tracking-widest mb-1">Daily Progress</p>
                                <h3 className="text-3xl font-black text-white">{getDayProgress()}%</h3>
                            </div>
                            <div className="w-16 h-16 relative flex items-center justify-center">
                                <div className="absolute inset-0 border-4 border-zinc-700 rounded-full"></div>
                                <div className="absolute inset-0 border-4 border-lime-400 rounded-full transition-all duration-1000" style={{ clipPath: `inset(${100 - getDayProgress()}% 0 0 0)` }}></div>
                        ...
                        <div className="space-y-3">
                            {habits.map(habit => {
                                ...
                                return (
                                    <div key={habit.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex items-center gap-4 transition-all hover:border-zinc-700">
                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isDone ? 'bg-lime-900/20 text-lime-400' : 'bg-zinc-800 text-zinc-600'}`}>
                                            <TimeIcon size={20} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h4 className={`font-bold text-lg truncate ${isDone ? 'text-zinc-500 line-through' : 'text-white'}`}>{habit.name}</h4>
                                            <div className="flex items-center gap-3 mt-1">
                                                <div className="flex items-center gap-1 text-xs font-mono text-zinc-500">
                                                    <Flame size={12} className={streak > 0 ? "text-orange-500" : "text-zinc-600"} /> {streak} DAY STREAK
                                                </div>
                                            </div>
                                        </div>
                                        <button onClick={() => onToggle(habit.id, formattedDateKey)} className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-300 transform active:scale-95 shadow-lg shrink-0 ${isDone ? 'bg-lime-400 text-black hover:bg-lime-300' : 'bg-zinc-800 text-zinc-600 hover:bg-zinc-700 border-2 border-dashed border-zinc-600'}`}>
                                            <Check size={32} strokeWidth={4} className={`transition-all ${isDone ? 'scale-100' : 'scale-50 opacity-0'}`} />
                                        </button>
                                    </div>
                                )
                            })}
                    ...
                    <div className="min-w-[800px]">
                        <table className="w-full border-collapse text-sm">
                            <thead className="bg-zinc-900 sticky top-0 z-20 shadow-lg">
                                <tr>
                                    <th className="p-4 text-left font-bold text-zinc-500 min-w-[200px] md:min-w-[240px] sticky left-0 bg-zinc-900 z-30 border-r border-zinc-800/50 uppercase text-xs tracking-wider">Protocol Name</th>
                                    <th className="p-2 text-center font-bold text-zinc-600 w-[70px] border-r border-zinc-800/50 text-[10px] uppercase tracking-wider">Target</th>
                                    {daysArray.map(day => (
                                    ...
                                    <th className="p-2 min-w-[70px] text-center text-zinc-500 font-bold sticky right-0 bg-zinc-900 z-20 border-l border-zinc-800/50 text-[10px] uppercase tracking-wider">Score</th>
                            ...
                            <tbody>
                                {habits.length === 0 ? (
                                    ...
                                    habits.map((habit, index) => {
                                        const totalCompleted = daysArray.reduce((acc, day) => acc + (habit.history?.[getDateKey(currentDate, day)] ? 1 : 0), 0);
                                            ...
                                            <tr key={habit.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors group relative">
                                                <td className="p-2 sticky left-0 bg-zinc-900 group-hover:bg-zinc-800/90 z-10 border-r border-zinc-800/50 transition-colors">
                                                    <div className="flex items-center gap-3 w-full">
                                                        <div className="flex-1 min-w-0 flex items-center gap-2">
                                                            <div className="flex items-center gap-1 text-orange-500 shrink-0" title={`Streak: ${streak}`}>
                                                            ...
                                                            <div className="text-zinc-600 shrink-0"><TimeIcon size={14} /></div>
                                                            <input type="text" value={habit.name} onChange={(e) => onUpdateField(habit.id, 'name', e.target.value)} className="w-full bg-transparent border-none focus:ring-0 text-zinc-200 font-bold placeholder-zinc-700 text-sm focus:text-lime-400 transition-colors truncate" placeholder="PROTOCOL NAME..." />
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="p-1 text-center border-r border-zinc-800/50">
                                                    <input type="number" value={(habit.goal && habit.goal > 0) ? habit.goal : ''} onChange={(e) => onUpdateField(habit.id, 'goal', parseInt(e.target.value) || 0)} className={`w-full text-center bg-transparent border-none focus:ring-0 font-mono text-xs font-bold ${habit.goal > 0 ? 'text-white' : 'text-zinc-600 italic'}`} placeholder={daysInMonth} />
                                                </td>
                                                {daysArray.map(day => {
                                                    const dateKey = getDateKey(currentDate, day);
                                                    const isDone = habit.history?.[dateKey];
                                                    const isFuture = isFutureDate(day);
                                                    const isPast = !isToday(day) && new Date(currentDate.getFullYear(), currentDate.getMonth(), day) < new Date(new Date().setHours(0,0,0,0));
                                                    const isDisabled = isFuture;
                                                    return (
                                                        <td key={day} className={`p-0 text-center border-r border-zinc-800/30 ${isToday(day) ? 'bg-lime-900/10' : ''} ${isDisabled ? 'bg-black/40' : ''}`}>
                                                            <button disabled={isDisabled} onClick={() => { if (isPast && !isDone) onRepairStreak(habit.id, dateKey); else onToggle(habit.id, dateKey); }} className={`w-full h-10 md:h-12 flex items-center justify-center transition-all duration-300 relative group/btn ${isDone ? 'bg-transparent' : 'hover:bg-zinc-800'} ${isDisabled ? 'cursor-not-allowed opacity-30' : 'cursor-pointer'}`}>
                                                                {isDone ? (
                                                                    <div className={`w-6 h-6 rounded-md flex items-center justify-center shadow-[0_0_15px_rgba(163,230,53,0.6)] transform scale-110 transition-transform duration-200 ${isDisabled ? 'bg-zinc-600 grayscale' : 'bg-lime-400 hover:scale-125 hover:rotate-3'}`}>
                                                                    ...
                                                                    <div className={`w-1.5 h-1.5 rounded-full transition-colors ${isDisabled ? 'bg-zinc-800' : 'bg-zinc-700 group-hover/btn:bg-zinc-600'}`}>
                                                                        {isPast && !isDone && <div className="opacity-0 group-hover/btn:opacity-100 absolute inset-0 flex items-center justify-center text-[8px] text-red-500 font-bold">X</div>}
                                                                    </div>
                                                                )}
                                                            </button>
                                                        </td>
                                                ...
                                                <td className="p-2 text-center font-bold text-xs border-l border-zinc-800/50 bg-zinc-900 group-hover:bg-zinc-800/90 sticky right-0 z-10">
                                                    <div className="flex flex-col items-center justify-center">
                                                        <span className={`text-base font-black ${isGoalMet ? 'text-lime-400' : 'text-white'}`}>{percent}%</span>
                                                        <span className="text-[9px] text-zinc-600 font-bold font-mono">{totalCompleted}/{effectiveGoal}</span>
                                                    </div>
                                                </td>
                                                <td className="p-2 text-center bg-zinc-900 group-hover:bg-zinc-800/90 sticky right-0 z-10">
                                                    <button onClick={() => handleDeleteClick(habit.id)} className={`transition-colors p-2 rounded-lg ${deleteConfirmId === habit.id ? 'bg-red-500 text-white animate-pulse' : 'text-zinc-600 hover:text-red-500 hover:bg-red-500/10'}`}>
...
const AnalyticsView = ({ habits, currentDate, onOpenShare, userProfile, onToggleHardcore }) => {
    ...
    const totalXP = habits.reduce((total, habit) => total + Object.values(habit.history || {}).filter(val => val === true).length, 0);
    const rankInfo = getRankInfo(totalXP);
    
    const lineData = Array.from({ length: daysInMonth }, (_, i) => {
        const day = i + 1;
        const count = habits.reduce((acc, h) => acc + (h.history?.[getDateKey(currentDate, day)] ? 1 : 0), 0);
        return { day, count };
    });
    ...
    for (let i = 0; i < 4; i++) {
        ...
        for (let d = startDay; d <= endDay; d++) {
            weekTotal += habits.reduce((acc, h) => acc + (h.history?.[getDateKey(currentDate, d)] ? 1 : 0), 0);
        }
    ...
    const totalCompletedActual = habits.reduce((acc, h) => {
        let habitCount = 0;
        for (let i = 1; i <= daysInMonth; i++) if (h.history?.[getDateKey(currentDate, i)]) habitCount++;
        return acc + habitCount;
    }, 0);
    const completionRate = totalGoals > 0 ? Math.round((totalCompletedActual / totalGoals) * 100) : 0;
    
    const sortedHabits = [...habits].sort((a, b) => {
        const getCount = (h) => { let c = 0; for (let i = 1; i <= daysInMonth; i++) if (h.history?.[getDateKey(currentDate, i)]) c++; return c; };
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 lg:gap-6 animate-fade-in pb-20">
            <div className="bg-zinc-900 p-6 rounded-3xl shadow-2xl border border-zinc-800 col-span-1 md:col-span-2 lg:col-span-2 flex flex-col justify-between min-h-[300px] relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Medal size={120} className="text-yellow-500" /></div>
                <div className="flex justify-between items-start">
                    ...
                    <button onClick={onOpenShare} className="p-2 bg-zinc-800 rounded-full text-zinc-400 hover:text-white hover:bg-zinc-700 transition-all"><Camera size={20} /></button>
                </div>
                <div className="mt-8">
                    ...
                    <div className="w-full h-4 bg-black rounded-full overflow-hidden border border-zinc-800">
                        <div className="h-full bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 animate-pulse" style={{ width: `${Math.min(100, rankInfo.progress)}%` }}></div>
                    </div>
                    <p className="text-[10px] text-zinc-500 mt-2 text-right">Complete habits to gain XP</p>
                </div>
            </div>
            
            <div className={`bg-zinc-900 p-6 rounded-3xl shadow-2xl border col-span-1 md:col-span-1 lg:col-span-2 min-h-[300px] flex flex-col relative overflow-hidden ${userProfile?.hardcoreMode ? 'border-red-500/30' : 'border-zinc-800'}`}>
                ...
                <div className="flex-1 flex flex-col justify-center space-y-4">
                    <div className="bg-black/40 p-4 rounded-xl border border-zinc-800 flex items-center justify-between">
                        ...
                        <div className="p-2 bg-zinc-900 rounded-lg text-zinc-500">
                            {userProfile?.archetype && (() => { const Icon = ARCHETYPES[userProfile.archetype.toUpperCase()]?.icon || UserCircle; return <Icon size={20} />; })()}
                        </div>
                    </div>
                    <div className="bg-black/40 p-4 rounded-xl border border-zinc-800 flex items-center justify-between">
                        <div>
                            <p className="text-xs text-zinc-500 font-mono mb-1">FORGIVENESS TOKENS</p>
                            <p className="text-lg font-black text-lime-400">{userProfile?.tokens !== undefined ? userProfile.tokens : 3}</p>
                        </div>
                        <div className="p-2 bg-zinc-900 rounded-lg text-lime-400"><Repeat size={20} /></div>
                    </div>
                    <div className="bg-black/40 p-4 rounded-xl border border-zinc-800 flex items-center justify-between">
                        <div>
                            <p className="text-xs text-zinc-500 font-mono mb-1 flex items-center gap-2">HARDCORE MODE {userProfile?.hardcoreMode ? <Skull size={12} className="text-red-500" /> : <ZapOff size={12} />}</p>
                            <p className={`text-sm font-bold ${userProfile?.hardcoreMode ? 'text-red-400' : 'text-zinc-400'}`}>{userProfile?.hardcoreMode ? 'Active: XP Penalty On' : 'Standard Protocol'}</p>
                        </div>
                        <button onClick={onToggleHardcore} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border transition-all ${userProfile?.hardcoreMode ? 'bg-red-500 text-white border-red-500 shadow-[0_0_10px_rgba(239,68,68,0.4)]' : 'bg-zinc-800 text-zinc-500 border-zinc-700 hover:text-white'}`}>
            ...
            <div className="bg-zinc-900 p-6 rounded-3xl shadow-2xl border border-zinc-800 col-span-1 md:col-span-2 lg:col-span-2 flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden group hover:border-lime-500/30 transition-colors duration-500">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Target size={80} className="text-lime-400" /></div>
                <h3 className="text-zinc-500 font-bold mb-2 uppercase text-[10px] tracking-[0.2em]">Efficiency Rating</h3>
                <div className="relative w-48 h-48">
                    {mounted && (
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={pieData} innerRadius={65} outerRadius={80} paddingAngle={5} dataKey="value" startAngle={90} endAngle={-270} stroke="none">
            ...
            <div className="bg-zinc-900 p-6 rounded-3xl shadow-2xl border border-zinc-800 col-span-1 md:col-span-2 lg:col-span-6 min-h-[300px] relative overflow-hidden">
                <div className="flex justify-between items-start mb-6"><h3 className="text-zinc-500 font-bold uppercase text-[10px] tracking-[0.2em] flex items-center gap-2"><TrendingUp size={14} className="text-fuchsia-500" /> Consistency Flow</h3></div>
                            ...
                            <LineChart data={lineData}>
                                <defs>
                                    <linearGradient id="lineColor" x1="0" y1="0" x2="1" y2="0">
                                        <stop offset="0%" stopColor="#a3e635" /><stop offset="100%" stopColor="#d946ef" />
                                    </linearGradient>
                                </defs>
                                ...
                                <Tooltip contentStyle={{ backgroundColor: '#18181b', borderRadius: '12px', border: '1px solid #27272a', color: '#fff' }} itemStyle={{ color: '#fff' }} cursor={{ stroke: '#52525b', strokeWidth: 1, strokeDasharray: '4 4' }} />
                                <Line type="monotone" dataKey="count" stroke="url(#lineColor)" strokeWidth={4} dot={false} activeDot={{ r: 8, fill: '#fff', stroke: '#d946ef', strokeWidth: 3 }} />
                            </LineChart>
                        </ResponsiveContainer>
                    )}
                </div>
            ...
            <div className="bg-zinc-900 p-6 rounded-3xl shadow-2xl border border-zinc-800 col-span-1 md:col-span-1 lg:col-span-3 min-h-[300px] overflow-hidden">
                <h3 className="text-zinc-500 font-bold mb-6 uppercase text-[10px] tracking-[0.2em] flex items-center gap-2"><Trophy className="text-yellow-500" size={14} /> Leaderboard</h3>
                <div className="space-y-4">
                    {sortedHabits.map((h, i) => {
                        ...
                        return (
                            <div key={h.id} className="flex items-center gap-4 group">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm shrink-0 transition-colors ${i === 0 ? 'bg-yellow-500 text-black shadow-[0_0_10px_rgba(234,179,8,0.4)]' : i === 1 ? 'bg-zinc-700 text-white' : i === 2 ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-900 border border-zinc-800 text-zinc-600'}`}>#{i + 1}</div>
                                <div className="flex-1 min-w-0">
            ...
            <div className="bg-zinc-900 p-6 rounded-3xl shadow-2xl border border-zinc-800 col-span-1 md:col-span-1 lg:col-span-3 min-h-[300px]">
                <h3 className="text-zinc-500 font-bold mb-6 uppercase text-[10px] tracking-[0.2em] flex items-center gap-2"><Activity size={14} className="text-cyan-400" /> Volume</h3>
                    ...
                    {mounted && (
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={weeklyData}>
                                ...
                                <YAxis axisLine={false} tickLine={false} tick={{ fill: '#52525b', fontSize: 10, fontWeight: 'bold' }} />
                                <Tooltip cursor={{ fill: '#27272a' }} contentStyle={{ backgroundColor: '#18181b', borderRadius: '12px', border: '1px solid #27272a', color: '#fff' }} />
...
export default function App() {
    // Mode can be: 'cloud', 'local', or null (uninitialized setup screen)
    const [appMode, setAppMode] = useState(() => safeStorage.get('app_mode'));
    ...
    // CRITICAL iOS FIX: Use Anonymous Auth directly. No popups, no indexedDB blocks.
    useEffect(() => {
        ...
        // Safety timeout in case iOS completely blocks Auth processing
    ...
    // Data Syncing Logic Based on Chosen Mode
    useEffect(() => {
        if (appMode === 'local') {
            const localData = safeStorage.get('me_supreme_habits');
            const localProf = safeStorage.get('me_supreme_profile');
            if (localData) setHabits(JSON.parse(localData));
            if (localProf) setUserProfile(JSON.parse(localProf));
            
            const parsedProf = JSON.parse(localProf || '{}');
            const parsedHabits = JSON.parse(localData || '[]');
            if (!parsedProf.archetype && parsedHabits.length === 0) {
        ...
        if (appMode === 'cloud' && user) {
            ...
            const unsubscribeProfile = onSnapshot(userDocRef, (docSnap) => {
                if (docSnap.exists()) {
                    ...
                    setUserProfile(data);
                    if (!data.archetype && habits.length === 0) setIsOnboarding(true);
                }
                setProfileLoading(false);
            }, (err) => {
            ...
            return () => { unsubscribeProfile(); unsubscribeHabits(); };
        }
    }, [appMode, user]);

    const handleCloudSync = async (identityData) => {
        ...
        try {
            ...
            safeStorage.set('app_mode', 'cloud');
            setAppMode('cloud');
        } catch (error) {
    ...
    const handleLocalMode = (identityData) => {
        const displayName = `${identityData.firstName} ${identityData.lastName}`;
        const localProfile = { displayName, tokens: 3 };
        safeStorage.set('me_supreme_profile', JSON.stringify(localProfile));
        safeStorage.set('app_mode', 'local');
        setUserProfile(localProfile);
        setAppMode('local');
    };

    const handleLogout = async () => {
        safeStorage.remove('app_mode');
        setAppMode(null);
        setHabits([]);
        setUserProfile(null);
    ...
    const handleAddHabit = async (name, goal, timeOfDay = 'anytime') => {
        const newHabitData = { name, goal, timeOfDay, createdAt: new Date().toISOString(), history: {} };
        if (appMode === 'local') {
            saveToLocal([...habits, { ...newHabitData, id: `local-${Date.now()}` }]);
            return;
        }
        if (!user) return;
        newHabitData.createdAt = serverTimestamp();
    ...
    const updateUserStats = async (updatedHabits) => {
        ...
        const todayScore = updatedHabits.reduce((acc, h) => acc + (h.history?.[todayKey] ? 1 : 0), 0);
        if (appMode === 'local') {
            updateLocalProfile({ todayScore, lastActive: Date.now() });
        } else if (user) {
            await setDoc(doc(db, 'artifacts', appId, 'users', user.uid), { todayScore, lastActive: serverTimestamp() }, { merge: true });
        }
    };
    ...
    const handleToggleHabit = async (habitId, dateKey) => {
        ...
        else { newHistory[dateKey] = true; isCompleting = true; }

        const newStreak = calculateStreak(newHistory);
        if (isCompleting && newStreak > 0 && newStreak % 7 === 0) {
            const currentTokens = userProfile?.tokens !== undefined ? userProfile.tokens : 3;
            if (appMode === 'local') updateLocalProfile({ tokens: currentTokens + 1 });
            else if (user) await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid), { tokens: currentTokens + 1 });
            setTokenToast(true); setTimeout(() => setTokenToast(false), 4000);
        }
        ...
        if (appMode === 'local') {
            ...
            await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'habits', habitId), { history: newHistory });
            setHabits(updatedHabits);
        }
    ...
    const confirmRepair = async () => {
        const currentTokens = userProfile?.tokens !== undefined ? userProfile.tokens : 3;
        if (pendingRepair && currentTokens > 0) {
            await handleToggleHabit(pendingRepair.habitId, pendingRepair.dateKey);
            if (appMode === 'local') updateLocalProfile({ tokens: currentTokens - 1 });
            else if (user) await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid), { tokens: currentTokens - 1 });
            setPendingRepair(null); setReflectionOpen(false);
        }
    };

    const handleUpdateField = async (habitId, field, value) => {
        const updatedHabits = habits.map(h => h.id === habitId ? { ...h, [field]: value } : h);
        setHabits(updatedHabits);
        if (appMode === 'local') saveToLocal(updatedHabits);
        else if (user) updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'habits', habitId), { [field]: value }).catch(e => console.error(e));
    };
    ...
    const handleDeleteHabit = async (habitId) => {
        ...
        setHabits(updatedHabits);
        if (appMode === 'local') {
            saveToLocal(updatedHabits); updateUserStats(updatedHabits);
        } else if (user) {
            try {
    ...
    const handleOnboardingComplete = async (archetypeId, defaultHabits) => {
        ...
        if (appMode === 'local') {
            updateLocalProfile({ archetype: archetypeId, manifesto: arch.manifesto });
            const newHabits = defaultHabits.map((h, i) => ({ ...h, id: `local-def-${Date.now()}-${i}`, createdAt: new Date().toISOString(), history: {} }));
            saveToLocal(newHabits);
        } else if (user) {
            await setDoc(doc(db, 'artifacts', appId, 'users', user.uid), { archetype: archetypeId, manifesto: arch.manifesto }, { merge: true });
            for (const h of defaultHabits) {
                await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'habits'), { ...h, createdAt: serverTimestamp(), history: {} });
            }
        }
    ...
    const handleToggleHardcore = async () => {
        ...
        if (appMode === 'local') updateLocalProfile({ hardcoreMode: newVal });
        else if (user) await setDoc(doc(db, 'artifacts', appId, 'users', user.uid), { hardcoreMode: newVal }, { merge: true });
    };

    const handleJoinSquad = async (squadId) => {
        if (appMode === 'local') updateLocalProfile({ squadId });
        else if (user) await setDoc(doc(db, 'artifacts', appId, 'users', user.uid), { squadId }, { merge: true });
    };

    if (loading) return <LoadingScreen message="BOOTING SYSTEM..." />;
    if (!appMode) return <LoginView onCloudSync={handleCloudSync} onLocalMode={handleLocalMode} isConfigured={firebaseConfig.apiKey !== "PLACEHOLDER"} />;
    if (profileLoading) return <LoadingScreen message="SYNCING PROFILE..." />;

    const totalXP = habits.reduce((total, habit) => total + Object.keys(habit.history || {}).length, 0);

    return (
        <ErrorBoundary>
            {/* The primary fix for iOS Safari blank screen: min-h-screen instead of dvh */}
            <div className="min-h-screen w-full bg-black text-zinc-100 font-sans flex flex-col selection:bg-lime-400 selection:text-black overflow-x-hidden">
                {isOnboarding && <OnboardingModal onComplete={handleOnboardingComplete} />}
                <ReflectionModal isOpen={reflectionOpen} onClose={() => setReflectionOpen(false)} onRecover={confirmRepair} tokens={userProfile?.tokens !== undefined ? userProfile.tokens : 3} />
                <UserGuide isOpen={showGuide} onClose={() => setShowGuide(false)} />
                <TokenToast visible={tokenToast} />
                <NewHabitModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onConfirm={handleAddHabit} />
                <ShareModal isOpen={isShareOpen} onClose={() => setIsShareOpen(false)} rankInfo={getRankInfo(totalXP)} totalXP={totalXP} habits={habits} />
                
                {/* Navbar */}
                <nav className="bg-zinc-900/80 backdrop-blur-lg border-b border-zinc-800 p-4 sticky top-0 z-50 print:hidden w-full">
                    <div className="max-w-7xl mx-auto flex justify-between items-center">
                        <div className="flex items-center gap-3 z-50 relative group cursor-pointer" onClick={() => setView('grid')}>
                        ...
                        <div className="hidden md:flex items-center gap-1 bg-black p-1 rounded-xl border border-zinc-800">
                            {['stats', 'grid', 'squad'].map(v => (
                                <button key={v} onClick={() => setView(v)} className={`px-4 py-2 rounded-lg text-sm font-bold uppercase transition-all ${view === v ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-white'}`}>
                                    {v}
                                </button>
                            ))}
                        </div>
                        <div className="flex items-center gap-4">
                            <div className="hidden sm:flex items-center bg-zinc-900 rounded-lg p-1 border border-zinc-800">
                                <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1))} className="p-1 hover:bg-zinc-800 rounded text-zinc-400 hover:text-lime-400 transition-colors"><ChevronLeft size={16} /></button>
                                ...
                                <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1))} className="p-1 hover:bg-zinc-800 rounded text-zinc-400 hover:text-lime-400 transition-colors"><ChevronRight size={16} /></button>
                            </div>
                            <button onClick={handleLogout} className="p-2 text-zinc-500 hover:text-red-500 transition-colors" title="Sign Out">
                                <LogOut size={20} />
                            </button>
                        </div>
                    </div>
                ...
                {/* Main Content Area */}
                <main className="flex-1 max-w-7xl w-full mx-auto p-2 md:p-6 print:p-0 print:max-w-none flex flex-col pb-24 md:pb-6 relative">
                    {view === 'stats' && (
                        <div className="flex-1 w-full">
                            ...
                            <AnalyticsView habits={habits} currentDate={currentDate} onOpenShare={() => setIsShareOpen(true)} userProfile={userProfile} onToggleHardcore={handleToggleHardcore} />
                        </div>
                    )}
                    {view === 'grid' && (
                        <div className="flex-1 h-full flex flex-col w-full min-h-[500px]">
                            <div className="flex items-center gap-2 mb-4 px-2">
                                <Zap className="text-yellow-400 fill-yellow-400" size={20} />
                                <h2 className="text-xl font-black tracking-tight text-white">ACTIVE PROTOCOLS</h2>
                            </div>
                            <TrackerView habits={habits} currentDate={currentDate} onToggle={handleToggleHabit} onRepairStreak={handleRepairStreak} onOpenNewHabit={() => setIsModalOpen(true)} onDelete={handleDeleteHabit} onUpdateField={handleUpdateField} />
                        </div>
                    )}
                    {view === 'squad' && (
                        <div className="flex-1 w-full">
                            <SquadView user={user} userProfile={userProfile} onJoinSquad={handleJoinSquad} appMode={appMode} />
                        </div>
                    )}
                </main>

                {/* Mobile Bottom Navigation */}
                <div className="md:hidden bg-zinc-900 border-t border-zinc-800 p-2 pb-6 fixed bottom-0 left-0 w-full z-40 flex justify-around">
                    <button onClick={() => setView('stats')} className={`flex flex-col items-center p-2 rounded-xl transition-all ${view === 'stats' ? 'text-lime-400' : 'text-zinc-600'}`}>
                        <BarChart2 size={24} strokeWidth={view === 'stats' ? 3 : 2} />
                        <span className="text-[10px] font-bold mt-1 uppercase">Stats</span>
                    </button>
                    <button onClick={() => setView('grid')} className="flex flex-col items-center -mt-6 z-50 relative">
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center border-4 border-black shadow-lg transition-all ${view === 'grid' ? 'bg-lime-400 text-black translate-y-1 shadow-[0_0_20px_rgba(163,230,53,0.4)]' : 'bg-zinc-800 text-zinc-500'}`}>
                            <Grid size={28} strokeWidth={3} />
                        </div>
                        <span className={`text-[10px] font-bold mt-2 uppercase ${view === 'grid' ? 'text-lime-400' : 'text-zinc-600'}`}>Grid</span>
                    </button>
                    <button onClick={() => setView('squad')} className={`flex flex-col items-center p-2 rounded-xl transition-all ${view === 'squad' ? 'text-lime-400' : 'text-zinc-600'}`}>
                        <Users size={24} strokeWidth={view === 'squad' ? 3 : 2} />
